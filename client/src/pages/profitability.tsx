...
import { Calculator, DollarSign, ArrowDownRight, Save, TrendingUp } from "lucide-react";
        ...
        <Card className="bg-primary text-primary-foreground border-none shadow-xl shadow-primary/20">
          <CardContent className="p-5">
            <div className="flex items-center gap-2 mb-3 opacity-90">
              ...
              <h3 className="text-sm font-medium">Profit Net (Livrées)</h3>
            </div>
            <p className="text-3xl font-display font-bold" data-testid="text-net-profit">{formatCurrency(aggregate.netProfit)}</p>
            <p className="mt-1 text-primary-foreground/80 text-xs">
              Marge: {profitMargin.toFixed(1)}% | {validOrders.length} commandes livrées
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="rounded-2xl border-border/50">
          <CardContent className="p-5">
            <div className="flex items-center gap-2 mb-3 text-destructive">
              ...
              <h3 className="text-sm font-medium">Coûts Totaux</h3>
            </div>
            <p className="text-3xl font-display font-bold text-foreground">
              {formatCurrency(aggregate.productCost + aggregate.shippingCost + aggregate.adSpend)}
            </p>
            <p className="mt-1 text-muted-foreground text-xs">
              Produit: {formatCurrency(aggregate.productCost)} | Livraison: {formatCurrency(aggregate.shippingCost)} | Pub: {formatCurrency(aggregate.adSpend)}
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="rounded-2xl border-border/50 bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-950/20 dark:to-orange-950/20">
          <CardContent className="p-5">
            ...
            <p className="text-2xl font-display font-bold text-foreground" data-testid="text-roas">
              ROAS: {aggregate.adSpend > 0 ? (aggregate.revenue / aggregate.adSpend).toFixed(2) : '∞'}x
            </p>
            <p className="text-sm font-semibold text-amber-700 dark:text-amber-400" data-testid="text-roi">
              ROI: {aggregate.adSpend > 0 ? ((aggregate.netProfit / aggregate.adSpend) * 100).toFixed(1) : '∞'}%
            </p>
          </CardContent>
        </Card>
      </div>
