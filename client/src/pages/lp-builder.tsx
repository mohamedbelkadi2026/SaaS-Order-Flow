...
import {
  ...
  BarChart3, ShoppingBag, ExternalLink, RotateCcw,
} from "lucide-react";
  ...
  async function generateCopy() {
    ...
    try {
      const result = await apiRequest("POST", "/api/lp-builder/generate-copy", {
        productName, priceDH: parseFloat(priceDH) || 0, description,
      });
      setCopy(result);
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-slate-400 text-sm mt-1">Créez des pages de vente qui convertissent — en 1 clic</p>
          </div>
        ...
        {pagesLoading ? (
          ...
          <div className="rounded-2xl border border-white/10 bg-white/5 p-12 text-center">
            ...
            <p className="text-slate-500 text-sm mb-6">Créez votre première landing page en moins de 5 minutes</p>
  ...
  if (step === "done") return (
    <div className="min-h-screen flex items-center justify-center p-6" style={{ background: "#0a1628" }}>
      <div className="max-w-md w-full text-center space-y-6">
        ...
        <p className="text-slate-400">Votre landing page est maintenant en ligne et prête à recevoir des commandes.</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_260px] gap-8">
            ...
            {step === 1 && (
              <>
                ...
                <Card className="border-white/10 bg-white/5 text-white">
                  <CardHeader className="pb-3">
                    <CardTitle className="text-sm font-bold uppercase tracking-widest flex items-center gap-2" style={{ color: GOLD }}>
                      <Upload className="w-4 h-4" /> Images (3 slots)
                    </CardTitle>
                    <p className="text-xs text-slate-500 mt-1">JPG/PNG/WEBP — max 10MB par image</p>
                  </CardHeader>
                  <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <ImageSlot label="Image Hero" desc="Vue principale, plein écran" value={heroImageUrl} onChange={setHeroImageUrl} />
                    <ImageSlot label="Image Features" desc="Produit en situation d'usage" value={featuresImageUrl} onChange={setFeaturesImageUrl} />
                    <ImageSlot label="Image Proof" desc="Résultats, avant/après, unboxing" value={proofImageUrl} onChange={setProofImageUrl} />
                  </CardContent>
                </Card>
            ...
            {step === 2 && (
              <>
                <Card className="border-white/10 bg-white/5 text-white">
                  <CardHeader className="pb-3">
                    <CardTitle className="text-sm font-bold uppercase tracking-widest flex items-center gap-2" style={{ color: GOLD }}>
                      <Wand2 className="w-4 h-4" /> Génération du texte de vente IA
                    </CardTitle>
                    <p className="text-xs text-slate-500 mt-1">Cliquez pour générer. Vous pouvez ensuite modifier chaque section.</p>
                  </CardHeader>
                  <CardContent>
                    <Button onClick={generateCopy} disabled={generatingCopy} className="w-full gap-2 font-bold mb-5"
                      ...
                      {generatingCopy ? <><Loader2 className="w-4 h-4 animate-spin" /> Génération en cours…</> : <><Wand2 className="w-4 h-4" /> Générer la Copy en Darija/Français</>}
                    </Button>

                    {/* Editable copy fields */}
                    {[
                      { key: "headline", label: "Titre principal (Hook)", placeholder: "Titre accrocheur court…" },
                      { key: "subheadline", label: "Sous-titre", placeholder: "Bénéfice principal en une phrase…" },
                      { key: "hook", label: "Phrase d'accroche", placeholder: "Touche la douleur du client…" },
                    ...
                    {/* Solution bullets */}
                    <div className="mb-4">
                      <label className="text-xs text-slate-400 font-semibold uppercase tracking-wide block mb-1.5">Bénéfices / Avantages (1 par ligne)</label>
                      <textarea rows={5}
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Button onClick={() => {
                    if (!copy.headline && !copy.cta) { toast({ title: "Générez ou remplissez la copy d'abord.", variant: "destructive" }); return; }
                    setStep(3);
                  }} className="gap-2 font-bold px-8" style={{ background: `linear-gradient(135deg, ${GOLD}, #e8b56a)`, color: NAVY }} data-testid="button-next-step-3">
          ...
          <div className="hidden lg:block">
            <div className="sticky top-8">
              ...
              <p className="text-[10px] text-slate-600 text-center mt-4">Le rendu final sera optimisé pour mobile</p>
            </div>
          </div>
        </div>
      </div>
